...
def _strict_clamp01(value: float) -> float:
    """Clamp value to strictly between 0 and 1 (exclusive) as required by hackathon."""
...
class SupportTriageEnvironment:
    ...
    def step(self, action: Action) -> StepResponse:
        ...
        if len(state.action_history) >= 2 and all(
            a == action_signature for a in state.action_history[-2:]
        ):
